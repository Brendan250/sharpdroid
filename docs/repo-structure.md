...
│   ├── FEX/                  FEXCore, and sixteen submodules of its own
│   └── libadrenotools/
│
...
both are **git submodules under `external/`**, pinned, and **never modified**:

| | pin | licence | why untouched |
| --- | --- | --- | --- |
...
| --- | --- | --- |
| a SharpEmu payload (`parity` or `performance`, linux-x64) | `dotnet publish`, driven by `scripts/package-build.ps1` | the fork checkout in the workspace |
...
| 3 | the workspace — `android-sdk`, `jdk-*` (globbed), `dotnet-sdk`, `sharpemu` |
