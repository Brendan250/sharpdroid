...
#
# **the debug identity is the default everywhere.** `com.mircowuffwuff.sharpemu.debug` is a separate
...
JAVA_PACKAGE = "com.mircowuffwuff.sharpemu"

# the release application id, and what a development build becomes instead.
RELEASE_ID = "com.mircowuffwuff.sharpemu"
...
RELEASE_LABEL = "SharpEmu"
DEBUG_LABEL = "SharpEmu Debug"
...
def application_label(package=None, release=False):
    ...
    if identity == DEBUG_ID:
        return DEBUG_LABEL
    return "SharpEmu (" + identity.rsplit(".", 1)[-1] + ")"
